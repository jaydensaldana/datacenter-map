...
================================================================
Reads manual data from a Google Sheet, fetches federal data, merges
everything, and writes combined.json for the map builder.
...
identified by header text (e.g., "Apr 2026"), never by position.

SOURCES (no API keys required):
  - Google Sheet: DC counts, tax incentives, regulations, regional markets
  - EIA: Annual electricity prices by state/sector (multiple fallback URLs)
...
def fetch_eia():
    """
    Try multiple EIA file sources for state-level annual electricity prices.
    Returns: {state_name: {year: {'res': price, 'com': price, 'ind': price}}}
    """
    print("\n--- EIA electricity prices ---")
    import openpyxl

    # Strategy: download Form EIA-861 detailed annual file
    # The "sales_revenue" file has by state/sector data for last several years
    # Format: data starts after header rows; columns include Year, State, Industry Sector, etc.
    candidates = [
        # Annual sales+revenue data — has state, sector, sales, revenue
        ("https://www.eia.gov/electricity/data/state/sales_annual.xlsx", "sales_annual"),
        ("https://www.eia.gov/electricity/data/state/avgprice_annual.xlsx", "avgprice"),
    ]
    ...
    for url, kind in candidates:
        ...
        try:
            ...
            for sheet_name in wb.sheetnames:
                ...
                # Find header row by scanning for "Year" + "State" combination
                hrow = None
                for rr in range(1, 10):
                    row_vals = [str(ws.cell(row=rr, column=c).value or '').lower().strip() for c in range(1, min(ws.max_column+1, 20))]
                    if 'year' in row_vals and 'state' in row_vals:
                        hrow = rr
                        break
                if not hrow:
                    continue
                ...
                for c in range(1, ws.max_column + 1):
                    ...
                    if v == 'year': cols['year'] = c
                    elif v == 'state': cols['state'] = c
                    elif 'industry sector' in v or 'sector' in v: cols['sector'] = c
                    elif 'residential' in v: cols['res'] = c
                    elif 'commercial' in v: cols['com'] = c
                    elif 'industrial' in v: cols['ind'] = c
                    elif 'price' in v and 'cents' in v: cols['price'] = c
                ...
                # Two formats:
                # Format A: separate columns for residential, commercial, industrial
                # Format B: long format with sector column + single price column
                has_wide = all(k in cols for k in ['res','com','ind'])
                has_long = 'sector' in cols and 'price' in cols
                ...
                for rr in range(hrow + 1, ws.max_row + 1):
                    ...
                    if not yr or not state_val: continue
                    state_str = str(state_val).strip()

                    # Map state abbreviation or name to canonical state name
                    ...
                    if state_name not in result:
                        result[state_name] = {}
                    if yr not in result[state_name]:
                        result[state_name][yr] = {}

                    if has_wide:
                        for sk in ['res','com','ind']:
                            v = safe_float(ws.cell(row=rr, column=cols[sk]).value)
                            if v is not None:
                                result[state_name][yr][sk] = v
                    elif has_long:
                        sec = str(ws.cell(row=rr, column=cols['sector']).value or '').strip().lower()
                        pr = safe_float(ws.cell(row=rr, column=cols['price']).value)
                        if pr is None: continue
                        if sec.startswith('res'): result[state_name][yr]['res'] = pr
                        elif sec.startswith('com'): result[state_name][yr]['com'] = pr
                        elif sec.startswith('ind'): result[state_name][yr]['ind'] = pr

                if result:
                    print(f"    Parsed {len(result)} states from {kind}")
                    return result
    ...
    print("  EIA: No data could be retrieved (will leave electricity columns empty)")
    return {}
...
def fetch_all_qcew():
    ...
    ly = CURRENT_YEAR - 1
    yrs = sorted({ly} | {ly - w for w in CHANGE_WINDOWS})
    out = {}
...
def build_combined(dc_hist, lat_dc, tax, regs, qcew, census, eia, mkts, lat_mkt):
    ...
    # Find latest EIA year (most data)
    eia_latest_yr = None
    if eia:
        all_yrs = set()
        for state_data in eia.values():
            all_yrs.update(state_data.keys())
        if all_yrs:
            eia_latest_yr = max(all_yrs)
    ...
    for state in sorted(dc_hist.keys()):
        ...
        # Electricity rates + % changes (if EIA data was successfully fetched)
        if eia_latest_yr and state in eia and eia_latest_yr in eia[state]:
            current_rates = eia[state][eia_latest_yr]
            for sec in ['res', 'com', 'ind']:
                cur_rate = current_rates.get(sec)
                e[f'{sec}_rate'] = cur_rate
                e[f'{sec}_year'] = eia_latest_yr
                for w in CHANGE_WINDOWS:
                    h_year = eia_latest_yr - w
                    h_rate = eia.get(state, {}).get(h_year, {}).get(sec)
                    e[f'{sec}_{w}yr_pct'] = pct_change(cur_rate, h_rate)
